...
const ShopComponent = () => {
	let data = JSON.parse(localStorage.getItem(`shopMovie`)) || [];
	...
	useEffect(() => {
		try {
			const shopMovie = JSON.parse(localStorage.getItem("shopMovie")) || [];
			setShop(shopMovie);
		} catch (e) {
			console.error("Error parsing shopMovie from localStorage", e);
			setShop([]);
		}
	}, []);
